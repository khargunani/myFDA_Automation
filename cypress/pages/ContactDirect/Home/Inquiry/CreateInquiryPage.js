class CreateInquiry{
    ...
    SelectInquiryType2Year = "Renewal: Cosmetic Registration 2 year"
...
openInquiryTab2Year(){
    ...
    createInquiry2Year(){
        ...
        cy.get(this.SelectsubInquiry).each(($ele) => {
            cy.log($ele.text());
            if ($ele.text() == "Cosmetic Registration 2 year") {
                cy.wrap($ele).click();
            }
        })
        cy.get(this.SelectContact).select("Priyanka Pathak(Main)");
    ...
    verifyInquiryIsUpdatedForOnline2Year(){
        cy.contains(this.InquiryTab).click();
        cy.contains(this.SelectInquiryType2Year).then(newTab=>{
            const hrefTab = newTab.prop('href')
            cy.visit(hrefTab);
        cy.contains(this.InquiryManagementLink).click();
        cy.get(this.InquiryStage).should('have.value','Closed-Won')
